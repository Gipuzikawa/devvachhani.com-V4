...
`src/motion/core.ts` is the foundation everything animates through. It is the **only** place that calls `gsap.registerPlugin` (ScrollTrigger, CustomEase, SplitText, ScrambleText, Flip, useGSAP); it re-exports `SplitText`/`Flip` so plugin access always routes through the core; it exports `DUR`/`EASE`/`REVEAL_RISE` constants that mirror the CSS motion tokens in `src/styles/tokens/effects.css` (the token eases are registered as named `CustomEase` curves — `ds-out`, `ds-in-out`, `ds-emph`, `ds-spring` — so CSS transitions and GSAP tweens share the literal same curves); it exports `DECODE_CHARS` (the one character well every scramble draws from) and `cssToken()` (token → computed value, for tweens that need a literal color); and it exports `prefersReducedMotion()`, the single JS-side reduced-motion check. Rules for anything new: register plugins in core only, draw timing from `DUR`/`EASE` only (add a CSS token first if a new value is needed), and consult `prefersReducedMotion()` instead of calling `matchMedia` directly — reduced-motion users get content in its final state, never gated.

**The reactive-text signature — "set & print":** the two typefaces carry one identity. Serif display text is *set* — it rises into place from behind per-line masks (`useSplitReveal`); mono labels are *printed* — they scramble-settle through `DECODE_CHARS` (`useDecode`). Reactive effects live on the display layer only (headlines, eyebrows, metadata, pull quotes, CTAs); body paragraphs never animate while being read, and no effect may reflow text mid-read (`useInkResolve` is color-only for exactly this reason). Decodes guard assistive tech with an `aria-label` of the final text while scrambling.
...
- `useFocusZoom` — scrubbed focus cycle for tall timeline blocks: zooms in (scale + opacity) as the block approaches the read line, holds completely static at scale 1 through the reading band, then zooms out and dims once the block's bottom clears it. The plateau is the point — text is never mid-scale or moving while actually being read. Transform-origin is the left edge so content stays anchored to the spine; only for blocks tall enough (>~25% viewport) that the approach/departure zones can't overlap. Used by the project page's timeline milestones (the wrapper excludes the spine node so it never scales off the 1px line).
- `useSplitReveal` — the serif signature: masked line/word/char rise via SplitText (`autoSplit` keeps line breaks honest across font load/resize). Self-animating `SectionHeading` titles, milestone titles, pull quotes, the footer sign-off.
- `useDecode` — the mono signature: every `[data-decode]` descendant scramble-settles as it crosses the viewport, with a cascade when several fire together. Eyebrows, index numbers, fact keys, colophon.
- `useInkResolve` — scrubbed per-line color resolve for *lead* display paragraphs ("the ink dries where you read"); never for body copy. About lead, F-35 closing reflection.
- `useMagnetic` — cursor lean + spring return for small interactive atoms; gated to `(hover: hover) and (pointer: fine)` via `gsap.matchMedia`, so touch devices get a static element.
...
The Home hero runs a one-off `useGSAP` timeline directly in `Home.tsx` — the reference for the "designed" feel: metas print, every headline character sets into its line (the split reverts to plain text once settled — split targets are the line spans, not the h1, so the revert keeps the recede's tween targets alive), then a scrub-linked recede peels the lines upward at different rates as the hero scrolls out. The Work index uses **Flip** for filter/layout changes: every card stays mounted, filtering toggles `display`, and `Flip.from` glides survivors to their new positions while entering/leaving cards fade through.

**Route transitions:** the original `.dc.html` prototype animated page swaps with a 3-direction disperse-out/slide-in transition (it was a hash-router SPA). That was deliberately dropped with real routes; what exists now is a deliberately *enter-only* transition (`RouteTransition` in `Layout`): the incoming page fades+rises over `--dur-slow`, no exit choreography (exit animations delay navigation), plus a cobalt hairline sweep across the top of the viewport (skipped on first load, which belongs to the hero; on article pages the sweep lands on the same line the reading-progress bar lives on). It is keyed by pathname so same-component navigations (e.g. between two future project slugs) still re-enter.
